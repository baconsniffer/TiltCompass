...
// ----- LCD
Adafruit_ST7735 tft = Adafruit_ST7735(CSPIN, DC, RSTPIN);
...
*/
float   Declination = -11.7000;                                             //  Degrees (For Trenton, Ontario)
int     Heading;
...
int     Mag_x_offset = 90,      Mag_y_offset = 239,     Mag_z_offset = 267;    // Hard-iron offsets
float   Mag_x_scale = 0.85,     Mag_y_scale = 0.71,     Mag_z_scale = 2.37;    // Soft-iron scale factors
float   ASAX = 1.21,            ASAY = 1.21,            ASAZ = 1.16;           // (A)sahi (S)ensitivity (A)djustment fuse ROM values.

// ----- LED
//const int LED = 2;                      // Status LED
...
void setup()
{
  ...
  */
  ...
  // ----- Start-up message
  tft.fillScreen(ST77XX_BLACK);                         // Clear the LCD
  tft.setCursor(30, 30);                                  // Set the LCD cursor to column 0 line 0
  tft.setTextColor(ST77XX_WHITE);                       // White text on black background
  tft.print("Tilt Compass");                           // Print text to screen
  tft.setCursor(50, 60);                                 // Set the LCD cursor to column 0 line 1
  tft.print("V1.0");                                    // Print text to screen
  tft.setCursor(45, 90);
  tft.print("ST7735");
  ...
  */
  ...
  tft.fillScreen(ST77XX_BLACK);                         //Clear the LCD
  tft.setCursor(1, 10);                                 //Set the LCD cursor to position
  tft.print("Hdg:");                                //Print text to screen
  tft.setCursor(1, 30);                                 //Set the LCD cursor to position
  tft.print("P:");                                      //Print text to screen
  tft.setCursor(1, 50);                                 //Set the LCD cursor to position
  tft.print("R:");                                      //Print text to screen
...
void calibrate_magnetometer()
{
  ...
  tft.fillScreen(ST77XX_BLACK);                                                    // Clear the LCD
  tft.setCursor(2, 50);                                           // Set the LCD cursor to column 0 line 0
  tft.print("Rotate Compass");                                    // Print text to screen
  tft.setCursor(2, 80);                                           // Set the LCD cursor to column 0 line 1
  ...
  for (int counter = 0; counter < 16000 ; counter ++)             // Run this code 16000 times
  {
    ...
    if (counter % 1000 == 0)tft.print(".");                        // Print a dot on the LCD every 1000 readings
...
void calibrate_gyro()
{
  // ----- Display calibration message
  tft.fillScreen(ST77XX_BLACK);                                          //Clear the LCD
  tft.setCursor(2, 10);                                  //Set the LCD cursor to position to position 0,0
  tft.print("Calibrating gyro");                        //Print text to screen
  tft.setCursor(2, 20);                                  //Set the LCD cursor to position to position 0,1
  ...
  {
    Loop_start = micros();
    if (counter % 125 == 0)tft.print(".");              //Print a dot on the LCD every 125 readings
    read_mpu_6050_data();                               //Read the raw acc and gyro data from the MPU-6050
  ...
  */
  ...
  switch (LCD_loop_counter)
  {
    ...
    case 1: // ----- Buffer heading reading
      ...
      sprintf(strHdg, "%03d", Heading);                       // Get heading with leading zeros
      tft.setCursor(DISPX + 11, 10);                           // Set the LCD cursor to display heading
      tft.print(strHdg);
      break;
    ...
    case 2: // ----- Buffer the pitch angle
      ...
      tft.setCursor(DISPX, 30);                               // Set the LCD cursor to display the pitch
      break;

    case 3: // ----- Display pitch sign
      (LCD_pitch_buffer < 0) ? tft.print("-") : tft.print("+");
      break;

    case 4: // ----- Display 1st pitch digit
      tft.print(abs(LCD_pitch_buffer) / 1000);
      break;

    case 5: // ----- Display 2nd pitch digit
      tft.print((abs(LCD_pitch_buffer) / 100) % 10);
      break;

    case 6: // ----- Display 3rd pitch digit
      tft.print((abs(LCD_pitch_buffer) / 10) % 10);
      break;
    ...
    case 8: // ----- Display 4th pitch digit
      tft.print(abs(LCD_pitch_buffer) % 10);
      break;

    ///////////////////// Roll //////////////////////////
    case 9: // ----- Buffer pitch reading
      LCD_roll_buffer = Gyro_roll_output * 10;                // Buffer the gyro roll angle because it will change
      tft.setCursor(DISPX, 50);                               // Set the LCD cursor to display roll angle
      break;

    case 10: // ----- Display roll sign
      (LCD_roll_buffer < 0) ? tft.print("-") : tft.print("+");
      break;

    case 11: // ----- Display 1st roll digit
      tft.print(abs(LCD_roll_buffer) / 1000);
      break;

    case 12: // ----- Display 2nd roll digit
      tft.print((abs(LCD_roll_buffer) / 100) % 10);
      break;

    case 13: // ----- Display 3rd roll digit
      tft.print((abs(LCD_roll_buffer) / 10) % 10);
      break;
    ...
    case 15: // ----- Display 4th roll digit
      tft.print(abs(LCD_roll_buffer) % 10);
      LCD_loop_counter = 0;
      break;
  }
}
